...
test_that("basic association tests work", {
  ...
  expect_no_error(
    fsh <- mirnaIntegration(obj, test = "association",
  ...
  expect_no_error(
    fshMidP <- mirnaIntegration(obj, test = "association",
  ...
  expect_no_error(
    bosch <- mirnaIntegration(sub, test = "association",
...
test_that("correlation analysis works for partially paired datasets", {
  ...
  unp <- createDummyData(nGenes = nrow(obj[["genes"]]),
