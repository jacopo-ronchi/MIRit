...
createDummyData <- function(nGenes = 100,
  ...
  if (paired == "all") {
    pSamp <- TRUE
    meta <- data.frame(primary = colnames(mirnaExpr),
    ...
    meta <- data.frame(primary = c(colnames(mirnaExpr), extraSamples),
                       mirnaCol = c(colnames(mirnaExpr), rep(NA, 4)),
                       geneCol = c(colnames(geneExpr)[seq(6)], NA, NA,
    ...
    pSamp <- FALSE
    colnames(geneExpr) <- c(paste("ptc", seq(8), sep = "_"),
                            paste("nth", seq(8), sep = "_"))
    meta <- data.frame(primary = c(colnames(mirnaExpr), colnames(geneExpr)),
