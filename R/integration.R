...
mirnaIntegration <- function(mirnaObj,
    ...
    if (max(dim(mirnaTargets(mirnaObj))) == 0) {
        stop(paste(
            "No targets are present within 'mirnaObj'!",
    ...
    if (geneDE(mirnaObj, param = TRUE)$method == "Manually added" &
        ...
        stop(
            paste(
                "Integration through 'fry' method is not available",
    ...
    if (test == "correlation" & pairedSamples(mirnaObj) == FALSE) {
        warning(
            paste(
                "You can't perform a 'correlation' analysis with",
    ...
    if (!is.character(corMethod) |
        ...
        stop(
            paste(
                "'corMethod' must be one of: 'pearson', 'kendall' and",
                "'spearman' (default)."
            ),
            call. = FALSE
        )
    ...
    if (!is.character(associationMethod) |
        ...
        stop(
            paste(
                "'associationMethod' must be one of: 'boschloo' (default),",
    ...
    if (test == "auto" & pairedSamples(mirnaObj) == TRUE) {
        ...
        message(paste(
            "Since data derive from different samples, a",
            "one-sided association test will be used."
        ))
    } else {
        intMethod <- test
        message(paste(
            "As specified by the user, a",
...
correlateMirnaTargets <- function(mirnaObj,
    ...
    if (!identical(mirnaSamples, geneSamples)) {
        ...
        if (length(unpaired) > 0) {
            ...
            warning(paste(
                "Some samples don't have expression values for both",
