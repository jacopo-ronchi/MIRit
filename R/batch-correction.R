...
batchCorrection <- function(mirnaObj,
  ...
  if (!is.null(batch)) {
    if (length(batch) == 1) {
      if (!is.character(batch) |
        ...
        stop(paste("'batch' must be a batch variable present in the metadata",
                   "(colData) of a MirnaExperiment object; or,",
                   "alternatively, it must be a character/factor object that",
                   "specifies the different batches."),
             call. = FALSE)
      }
    } else {
  ...
  if (!is.null(batch2)) {
    if (length(batch2) == 1) {
      if (!is.character(batch2) |
        ...
        stop(paste("'batch2' must be a batch variable present in the metadata",
                   "(colData) of a MirnaExperiment object; or,",
                   "alternatively, it must be a character/factor object that",
                   "specifies the different batches."),
             call. = FALSE)
  ...
  if (!is.null(covariates)) {
    if (is.character(covariates)) {
      if (any(!covariates %in% colnames(colData(mirnaObj)))) {
        stop(paste("'covariates' must be either a vector of column names",
                   "present in the metadata (colData) of a MirnaExperiment",
                   "object; or, alternatively, it must be a numeric vector or",
                   "matrix object that specifies the covariates to",
                   "correct for."),
             call. = FALSE)
      }
  ...
  if (!is.null(mirnaObj@metadata$uncorrectedMatrices[[assay]])) {
    warning(paste("Batch effect correction has already been performed for",
  ...
  if (is.character(covariates)) {
    smCov <- sm[, covariates]
    allNumeric <- vapply(colnames(smCov), function(x) is.numeric(smCov[, x]),
                         FUN.VALUE = logical(1))
    if (any(allNumeric == FALSE)) {
      stop(paste("'covariates' must only contain numeric variables!",
                 "See ?batchCorrection"),
           call. = FALSE)
    }
