...
MicroRNAs are small non-coding transcripts with an average length of 22 nucleotides that have a pivotal role in gene expression regulation. These non-coding RNAs take part in several biological processes and are dysregulated in a large proportion of human diseases. Generally, miRNAs act by binding some partially complementary regions in the 3' UTR of their target mRNAs, hence preventing translation and causing transcript degradation. Therefore, miRNAs are usually considered as master negative regulators of gene expression. The fine gene regulation put in place by miRNAs is critical for the normal functioning of cells, especially considering that over 60% of human genes are regulated by these small transcripts.

Given the above, a complete characterization of miRNA regulatory networks results crucial to explore the molecular networks that may lead to the insurgence of complex diseases. Unfortunately, different obstacles need to be faced before linking miRNA dysregulations to their biological outcomes. Firstly, the identification of miRNA targets through computational prediction is still affected by several false positives, thus undermining the validity of downstream analyses. In this regard, if both miRNA and mRNA expression are available, one option is to only consider miRNA-mRNA pairs that appear negatively correlated, hence retaining just the interactions that influence the expression values of target genes. However, few tools allow to systematically link expression changes of miRNAs with those of their targets through correlation analyses, and this even worse when exploring miRNA-target associations for unpaired data, with no existing software able to combine miRNA profiling with gene expression measurements obtained from different individuals. Furthermore, there are no currently available options for thoroughly interpreting the biological consequences of miRNA dysregulations, thus limiting the ability to identify the affected pathways and to reconstruct the compromised molecular networks. Lastly, the lack of a unified framework for integrative miRNA-mRNA analyses lessens the reproducibility of these analyses, and hinders the understanding of pathogenic mechanisms involving miRNAs.

To tackle these limitations, we developed MIRit, an all-in-one framework that provides flexible and powerful methods for performing integrative miRNA-mRNA multi-omic analyses from start to finish. In particular, MIRit includes multiple modules that allow to perform:
...
devtools::install_github('jacopo-ronchi/MIRit')
```
...
As anticipated above, the standard MIRit workflow involves multiple steps, namely:

1.  Data preparation and differential expression;
2.  Functional enrichment analysis of genes;
3.  Definition of miRNA targets;
4.  Integrative analysis of miRNA and gene expression values;
5.  Topological analysis of compromised regulatory networks;
6.  Visualization of network dysregulations.
...
Once we have expression matrices in the proper format, we need to inform MIRit about the samples in study and the biological conditions of interest. To do so, we need to create a `data.frame` that will hold metadata of patients in analysis, such as disease condition and subject ID. Specifically, this `data.frame` must contain:
...
In the end, this object should look like that reported in Table \@ref(tab:metaDf).

```{r metaDf, echo=FALSE}
## display the data.frame object that stores sample metadata
knitr::kable(meta, digits = 2, caption = "Sample metadata. A data.frame object that maps each column in expression matrices to the relative individual specified in 'primary' column.")
```

## Create a `MirnaExperiment` object

At this point, after setting up expression matrices, and after defining sample metadata, we need to create an object of class `MirnaExperiment`, which is the main class used in MIRit to store all the data that are necessary for the integrative miRNA-mRNA analysis. In particular, this class extends the `MultiAssayExperiment` class from the homonym package [@ramos_software_2017] to store expression levels of both microRNAs and genes, differential expression results, miRNA-target pairs and integrative miRNA-gene analysis.
...
geneMDS <- plotDimensions(experiment, assay = "genes", condition = "disease",
                          title = "MDS plot for genes")
mirnaMDS <- plotDimensions(experiment, assay = "microRNA", condition = "disease",
...
Additionally, when we run these functions, there are different parameters that we need to provide, including:
...
In the end, the analysis reported 265 differentially expressed genes, and 40 differentially expressed miRNAs.
...
Therefore, let's access the results of differential expression analysis in thyroid cancer for both miRNAs and genes.
...
To have an idea of the information included in these tables, differential expression results for genes are reported in Table \@ref(tab:geneDeTab).

```{r geneDeTab, echo=FALSE}
## display differential expression results for genes
knitr::kable(head(deGenes), digits = 2, caption = "Gene differential expression. A table listing the first 5 differentially expressed genes between thyroid cancer and normal thyroid.")
```
...
As evidenced in Figures \@ref(fig:volcano)A and \@ref(fig:volcano)B, upregulated and down regulated features are balanced for genes, while for miRNAs there are some species that show a consistently higher log2 fold change. Notably, the `plotVolcano()` function has also a `label` parameter that can highlight specific features or the most significant elements.

### Produce differential expression bar plots

Finally, if we are interested in specific genes/miRNAs, MIRit implements the `plotDE()` function that allows to represent expression changes of specific features as box plots, bar plots, or violin plots. In our example, we can use this function to visualize the two most significant miRNAs through boxplots.

```{r mirBox, fig.wide=FALSE, fig.cap="Differential expression box plots for the two most significant miRNAs."}
## craete a box plot for the most significant miRNA
topMir <- plotDE(experiment, features = deMirnas$ID[1], nameAsTitle = TRUE)

## create a boxplot for the second most significant miRNA
secondMir <- plotDE(experiment, features = deMirnas$ID[2], nameAsTitle = TRUE)

## plot graphs side by side
ggpubr::ggarrange(topMir, secondMir, nrow = 1, labels = "AUTO",
                  common.legend = TRUE, legend = "bottom")
```

Moreover, we can visualize expression changes of different genes involved in the normal functioning of thyroid gland, including thyreoglobulin (TG), which is the thyroid hormone; thyroid peroxidase (TPO), which is an enzyme needed for TG production; iodothyronine deiodinase 2 (DIO2), which is involved in the activation of TG; and PAX8, which is a transcription factor needed for TG transcription. Note that we use the `linear = FALSE` option to plot data in log scale.
...
In thyroid cancer, as evidenced in Figure \@ref(fig:thyroidBars), we assist to a heavy downregulation of genes involved in the synthesis and activation of thyroid hormone. Notably, this phenomenon could be clinically relevant, as it may determine the insurgence of iodine-resistance.
...
Here, $N$ is the total number of genes tested, $M$ is the number of genes that are annotated to a particular gene set, $n$ is the number of differentially expressed genes, and $k$ is the number of differentially expressed genes that are annotated to the gene set. After calculating p-values for each gene set, and after multiple testing correction, significant categories represent the biological processes that vary across the tested conditions.

Additionally, another available approach is the **gene set enrichment analysis** [@subramanian_gene_2005], often refereed to with the acronym **GSEA**. The main limitation of ORA is that small differences in gene expression are never significant. On the contrary, GSEA is suitable to find categories whose genes change in a small but coordinated way. The GSEA algorithm takes as input a list of genes ranked with a particular criterion, and then walks down the list to evaluate whether members of a given gene set are normally distributed or are mainly present at the top or at the bottom of the list. To check this out, the algorithm uses a running-sum that increases when finding a gene belonging to a given category, and decreases when a gene not contained in that specific set is found. The maximum distance from zero occurred in this running score is defined as the enrichment score (ES). To estimate the statistical significance of enrichment scores, a permutation test is performed by swapping gene labels annotated to a gene set. Then, the enrichment score is calculated for the permuted data and the p-value is then defined on the basis of this permutation analysis. Finally, p-values are computed for all the available gene sets and then corrected for multiple testing.

Even though GSEA is arguably the most commonly used approach for functional enrichment, @wu_camera_2012 demonstrated that inter-gene correlations may affect the reliability of functional enrichment analyses. To overcome this issue, they developed **Correlation Adjusted MEan RAnk gene set test (CAMERA)**, which is another competitive test used for functional enrichment analysis of genes. The main advantage of this method is that it adjusts the gene set test statistic according to inter-gene correlations.

## Available databases and categories {#categories}

As described above, functional enrichment analysis relies on gene sets, which consist in collections of genes that are annotated to specific functions or terms. Independently from the strategy used for the analysis, functional enrichment methods need access to these properly curated collections of genes.

In this regard, in the effort of providing access to a vast number of these resources, MIRit uses the `r Biocpkg("geneset")` package to support multiple databases, including Gene Ontology (GO), Kyoto Encyclopedia of Genes and Genomes (KEGG), MsigDB, WikiPathways, Reactome, Enrichr, Disease Ontology (DO), Network of Cancer Genes (NCG), DisGeNET, and COVID-19.

However, the majority of these databases contain multiple categories. For each database, the available categories are:
...
- Reactome;
- Enrichr:
	+ All avaliable gene sets can be listed through `geneset::enrichr_metadata`
- Disease Ontology (DO);
...
Now that we have introduced functional enrichment techniques and available gene sets, we are finally ready to perform the analysis. The main function used for this purpose is `enrichGenes()`, which requires as input several parameters, including:
...
As we can see from the output, the analysis reported 11 significant terms for the enrichment of downregulated genes. MIRit performs ORA separately for upregulated and downregulated genes, as it has been shown that this is more powerful compared to enriching all DE genes [@hong_separate_2014-1]. Therefore, when we use ORA, `enrichGenes()` returns a `list` containing two objects of class `FunctionalEnrichment`, one storing enrichment results for upregulated genes, and one for downregulated genes.
...
After running the `enrichGenes()` function, we get `FunctionalEnrichment` objects holding the results of enrichment analyses. To access the full table containing significantly effected biological functions, we can use the `enrichmentResults()` function.

In our case, we can check the results of our GSEA analysis to investigate the human pathways that result affected in thyroid cancer. In Table \@ref(tab:gseaTab) we can see the output of `enrichmentResults(gse)`.
...
Notably, we observe a strong depletion of the "Thyroid hormone synthesis" pathway, which is fundamental for the proper functioning of human thyroid. Moreover, this result is concordant with the previously described downregulation of multiple factors involved in the production and activation of thyroid hormone (Figure \@ref(fig:thyroidBars)).
...
Following our example, we can visualize the results of the ORA that we performed in Section \@ref(enrichment) through a simple dotplot.

```{r oraDot, fig.wide=TRUE, fig.cap="ORA results for downregulated genes. The enrichment of downregulated genes through the gene sets provided by GO database."}
## create a dot plot for ORA
enrichmentDotplot(ora$downregulated, title = "Depleted functions")
```

Also in this case, in concordance with GSEA results, we appreciate the depletion of genes responsible for the metabolism and generation of thyroid hormone, further highlighting the impairment of this pathway in thyroid cancer (Figure \@ref(fig:oraDot)).
...
First of all, we need to identify the Experimental Factor Ontology (EFO) identifier of a given disease of interest. To do so, MIRit provides the `searchDisease()` function. For example, let's identify the relevant EFO ID for Alzheimer disease.
...
As we notice, the relevant EFO trait is "Alzheimer disease".
...
As reported in the MIRit paper, we detected the presence of the rs2632516 polymorphism within the miR-142 host gene on the negative strand. In this regard, this variant has been previously associated to the downregulation of miR-142, as we observed in our analysis.
...
Now we are going to discuss the supported integration strategies separately, with a brief explanation of the different methodologies.
...
2.  Calculate the correlation coefficient for each miRNA-target pair on the basis of their expression values;
...
As we can see from the output, a statistically significant correlation between 215 miRNA-target pairs has been found.
...
Additionally, for correlation analyses, MIRit allows to graphically represent inverse correlations through a scatter plot. Indeed, we can make use of the `plotCorrelation()` function to display the correlation between specific miRNA-target pairs. For example, the correlation analysis performed in Section \@ref(correlation) revealed how hsa-miR-146b-5p, the most upregulated miRNA, is inveresely correlated with the expression of DIO2, which is crucial for thyroid hormone functioning. Furthermore, it has also emerged that hsa-miR-146b-3p results negatively correlated with PAX8, which directly induces TG transcription.

```{r corPlot, fig.wide=FALSE, fig.cap="Correlation between miRNAs and key thyroid genes. These plots suggest that the upregulation of miR-146b-5p and miR-146b-3p may be responsible for the downregulation of DIO2 and PAX8, respectively."}
...
For unpaired data we cannot directly quantify the influence of miRNA expression on the levels of their targets, because we don't have any sample correspondence between miRNA and gene measurements. However, **one-sided association tests** can be applied in these cases to evaluate if targets of down-regulated miRNAs are statistically enriched in up-regulated genes, and, conversely, if targets of up-regulated miRNAs are statistically enriched in down-regulated genes. In this regard, to estimate the effects of differentially expressed miRNAs on their target genes, MIRit can use two different one-sided association tests, namely:
...
After that the contingency table has been defined, a Fisher's exact test p-value can be calculated through Equation \@ref(eq:fisher).
...
The `mirnaIntegration()` function takes care of the whole process and computes p-values for each differentially expressed miRNA. Lastly, Fisher's p-values are adjusted for multiple testing and significant results are reported. Additionally, it is also possible to compute Fisher's p-values with **Lancaster's mid-p adjustment**, since it has been proven that it increases statistical power while retaining Type I error rates.
...
For example, we could use the Boschloo's exact to evaluate the inverse association between miRNA and gene expression values through a simple call to `mirnaIntegration()` function. However, note that in our example we are dealing with paired data, and therefore the approach of choice is the correlation analysis.
...
Finally, after performing the association analysis, results can be accessed through the `integration()` function in the same way as we can do for correlation analysis.
...
After running this function, pathways with less than 10% of nodes with expression measurements are removed. This option can be chnaged by specifying the `minPc` parameter.
...
As a result of the analysis, an object of class `IntegrativePathwayAnalysis` storing the results of TAIPA is returned.

Notably, the user can change the behavior of the `topologicalAnalysis()` in several ways. For example, the `pCutoff` and `pAdjustment` parameters can be used to change the significance threshold and the multiple testing correction method, respectively. Moreover, the `nPerm` parameter can be tweaked to change the number of permutations to use for evaluating statistical significance. In this regard, we recommend to use at least 10000 permutations, with no less than 1000.
...
Eventually, the resulting `data.frame` will appear as shown in Table \@ref(tab:taipaRes).

```{r taipaRes, echo=FALSE}
## display the results of TAIPA
knitr::kable(head(perturbedNetworks), digits = 4, caption = "TAIPA results. A table listing the most impaired miRNA-mRNA networks in thyroid cancer.")
```

## Visualize the impaired networks within biological pathways

Similarly to functional enrichment analyses, we can plot perturbed miRNA-mRNA networks as dot plots. To do so, the `integrationDotplot()` function can be used. In particular, we will graphically represent the most perturbed pathways in thyroid cancer.
...
```{r thyroidNetwork, fig.wide=TRUE, fig.cap="Impaired miRNA-mRNA regulatory netowrk prevents thyroid hormone synthesis in thyroid cancer. The network created by MIRit suggests that the upregulation of miR-146b-5p and miR-146b-3p may be responsible for diminished expression of PAX8, which in turn causes reduced transcription of thyroid hormone."}
## plot the impaired network responsible for reduced TG synthesis
visualizeNetwork(taipa, "Thyroid hormone synthesis")
```

From Figure \@ref(fig:thyroidNetwork) we appreciate how the strong overexpression of miR-146b-5p and miR-146b-3p causes the reduced expression of PAX8, which then determines a diminished transcription of thyroid hormone.
