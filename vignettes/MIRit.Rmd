...
load("~/Documenti/MIRtools/geneCounts.RData")
load("~/Documenti/MIRtools/mirnaCounts.RData")
source("~/Documenti/MIRtools/R/AllGenerics.R")
source("~/Documenti/MIRtools/R/AllClasses.R")
source("~/Documenti/MIRtools/R/association.R")
source("~/Documenti/MIRtools/R/enrichment.R")
source("~/Documenti/MIRtools/R/integration.R")
source("~/Documenti/MIRtools/R/show-methods.R")
source("~/Documenti/MIRtools/R/targets.R")
source("~/Documenti/MIRtools/R/utils.R")
source("~/Documenti/MIRtools/R/visualization.R")
```
...
```{r loadPackage, message=FALSE, eval=FALSE}
## load MIRit
library(MIRit)
```
...
carried out trough numerous specialized packages that are designed to identify
...
the basis of disease condition, thus suggesting that this factor is the major
factor that drives data variability. This is exactly what we want, since we are
willing to spot the differences between cancer and normal tissue. If this wasn't
the case, we would need to apply some batch effect correction strategies like
SVA [@leek_capturing_2007] and ComBat [@johnson_adjusting_2007] to remove
unwanted sources of variation. This is relevant because before running MIRit
expression matrices must be batch effect free.
...
expression values with the negative binomial distribution.
...
geneExpr <- cpm(genes, normalized.lib.sizes = TRUE)

## extract normalized counts for miRNA expression
mirnaExpr <- cpm(mirnas, normalized.lib.sizes = TRUE)
```
...
As pointed out, different tools can be used to asses differential expression in
...
in respect to normal thyroid tissue. To do this, we will use the `gseaGenes()`
...
Now that we carried out both ORA and GSEA, we can visualize the results of these
analysis with dotplots and ridgeplots. Dotplots are available for both ORA and
GSEA, while ridgeplots can be used exclusively on GSEA results. Firstly, to
...
Then, we can generate dotplots for enrichment results trough the function
`dotplot()`. The default option is to print the top ten enriched terms, but we
...
produce a dotplot for GSEA results that puts in evidence which biological
functions are increased and which are depleted. For this purpose, we will set
the `splitDir` parameter of the `mirnaDotplot()` function to `TRUE`.

```{r enrichmentMirnas, fig.cap="MiRNAs functional enrichment. A dotplot that sumarizes the results of miRNA GSEA through miEAA 2.0.", fig.dim=c(11, 4), fig.wide=TRUE}
...
The microRNA-mRNA interactions reported by this function can derive by both
predicted and validated interactions, or by a combination of predicted and
...
experiment <- readRDS("~/Documenti/MIRtools/test_2.RDS")
```
...
Several statistical coefficients can be used to weight the degree of a
...
one for determining the association between miRNA and gene expression. However,
it assumes that the relationships between miRNA and gene expression values is
...
the expression of its target. However, it was recently proven that in some
...
The Fisher's exact test is a statistical procedure that tests the association
...
: (#tab:contingency) The 2x2 contingency table that MIRit uses for the Fisher's
...
p-values are corrected for multiple testing and significant results are
reported.
...
After identifying the miRNA-target pairs that are significantly
correlated/associated, we are going to characterize these regulatory axes within
the impaired biological pathways, in order to reach a complete insight of the
affected cellular processes. MIRit is able to work with the most popular
databases of biological pathways, namely KEGG, Reactome and WikiPathways, and
provides a unified way to recreate the miRNA-gene networks that regulate most
biological functions. In particular, MIRit:
...
pathways posses an important therapeutic role, as the chemotherapy with
...
in the graph. The first option is to set the the `layout` parameter to `stress`
for arranging nodes according to an optimized version of the stress-minimization
...
conclude that a dysregulation born by these two miRNAs may be responsible for
the reduction of PAX8, which in turn is not able to activate the transcription
of numerous thyroid factors, including thyroid hormone. This compromised
regulatory axis may account for radioactive iodine-resistance in thyroid cancer,
and it has been further investigated and experimentally validated by the authors
of the study.
