...
samples used for miRNA profiling and for gene expression analysis. Specifically, this data.frame must contain:
...
For unpaired samples, NAs must be used for missing entries in `mirnaCol`/`geneCol`.
...
In our simple example, both miRNA and gene expression matrices have the same column names. However, the final `data.frame` containing sample metadata should look like this:

```{r sampleMap, echo=FALSE}
meta
```

Now we are ready to create a `MirnaExperiment` object, which will be the only thing needed for performing the integrative analysis with MIRit.
