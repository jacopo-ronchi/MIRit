...
geneExpr <- cpm(genes, normalized.lib.sizes = TRUE, log = TRUE)

## extract normalized counts for miRNA expression
mirnaExpr <- cpm(mirnas, normalized.lib.sizes = TRUE, log = TRUE)
```
...
In the end, the analysis reported 265 differentially expressed genes, and 40
...
Firstly, we need to inform this function about samples analyzed in the study.
To do so, we need to create a `data.frame` object containing information about
samples used for miRNA profiling and for gene expression analysis. Specifically,
this data.frame must contain:

- A column named `primary`, specifying an identifier for each sample;
- A column named `mirnaCol`, containing the column names used for each sample in
the `mirnaExpr` object;
- A column named `geneCol`, containing the column names used for each sample in
the `geneExpr` object;
- Other eventual columns that define specific sample metadata, such as disease
condition, age, sex and so on...

For unpaired samples, NAs must be used for missing entries in
`mirnaCol`/`geneCol`.

```{r sampleMap}
## create a sample map for miRNA and gene expression
...
In our simple example, both miRNA and gene expression matrices have the same
column names. However, the final `data.frame` containing sample metadata should
look like this:
...
tissue. Indeed, we observe that several genes involved in this pathway, such as
TPO, TG, PAX8, SLC26A4, and CANX, result heavily downregulated in thyroid 
cancer. In this regard, we can use the `plotDE` function included in MIRit to
visualize miRNA/gene dysregulations through boxplots, barplots, or violinplots,
as shown in Figure \@ref(fig:deBox).

```{r deBox, fig.cap="DEGs boxplot. A boxplot displaying downregulated genes involved in thyroid hormone production.", fig.dim=c(8, 6.5)}
## produce a boxplot with significance levels
plotDE(experiment, features = c("TPO", "TG", "PAX8", "SLC26A4", "CANX"), condition = "disease", graph = "boxplot")
```

Moreover, in addition to show gene dysregulations, the `plotDE()` function can 
be used to represent miRNA dysregulations as well, or also a combination of 
miRNA and gene dysregulation. For example, in Figure \@ref(fig:deBoxMirna) we 
reported the most upregulated and the most downregulated miRNA.

```{r deBoxMirna, fig.cap="Differentially expressed miRNAs in thyroid cancer. The left figure represents the most upregulated miRNA in thyroid cancer, while the figure on the right reports the most repressed one.", fig.dim=c(8, 6.5)}
## extract miRNA differential expression
mirDf <- mirnaDE(experiment)

## get the most upregulated and the most downregulated miRNA
upMir <- mirDf$ID[mirDf$logFC == max(mirDf$logFC)]
downMir <- mirDf$ID[mirDf$logFC == min(mirDf$logFC)]

## produce a baplot with significance levels for the most upregulated miRNA
topMir <- plotDE(experiment, features = upMir, condition = "disease", graph = "barplot")

## produce a baplot with significance levels for the most downregulated miRNA
botMir <- plotDE(experiment, features = downMir, condition = "disease", graph = "barplot", colorScale = c("hsa-miR-873-5p" = "steelblue"))

## plot the two figures side by side
ggpubr::ggarrange(topMir, botMir)
```
...
gene expression. Moreover, since miRNAs mainly act as negative regulators, only
...
exhibit a negative correlation with a Spearman's coefficient lower than -0.5
and with FDR lower than 0.05.
...
significant correlation between 204 miRNA-target pairs has been found.
Among the 265 differentially expressed genes, 100 are significantly correlated
with at least one differentially expressed miRNA. This is particularly
interesting because it tells us that miRNA dysregulations account for nearly
50% of gene alterations found in thyroid cancer.
...
namely hsa-miR-146b-5p and hsa-miR-34a-5p, as reported in \@ref(fig:paxBar). 
Based on these interactions, we could conclude that a dysregulation engendered 
by these two miRNAs may be responsible for the reduction of PAX8, which in turn
is not able to activate the transcription of numerous thyroid factors, including thyroid hormone. This compromised regulatory axis may account for radioactive iodine-resistance in thyroid cancer, as highlighted by the authors of the study.

```{r paxBar, fig.cap="PAX8, miR-146b-5p and miR-34a-5p differential expression. A barplot showing the differential expression of these miRNA-target pairs in thyroid cancer.", fig.dim=c(8, 6.5)}
## produce a barplot with significance levels
plotDE(experiment, features = c("PAX8", "hsa-miR-146b-5p", "hsa-miR-34a-5p"), condition = "disease", graph = "barplot")
```

Additionally, MIRit allows to produce a scatter plot that shows miRNA-target
correlation. Indeed, we can use the `plotCorrelation()` function to create a
scatter plot for miRNA and gene expression values, and to fit a linear/monotonic
regression line, as reported in Figure \@ref(fig:corPlot).

```{r corPlot, fig.cap="Correlation between hsa-miR-34a-5p, hsa-miR-146b-5p and PAX8 expression values.", warning=FALSE, fig.dim=c(8, 4)}
## plot the correlation between miR-34a-5p and PAX8
plotA <- plotCorrelation(experiment,
                mirna = "hsa-miR-34a-5p",
                gene = "PAX8",
                condition = "disease")

## plot the correlation between miR-146b-5p and PAX8
plotB <- plotCorrelation(experiment,
